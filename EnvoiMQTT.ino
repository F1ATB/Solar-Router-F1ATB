...
bool testMQTTconnected()
{
  ...
  { // si le mqtt n'est pas connecté (utile aussi lors de la 1ere connexion)
    ...
    if (clientMQTT.connect(MQTTdeviceName.c_str(), MQTTUser.c_str(), MQTTPwd.c_str(), AvailableTopic, 2, true, "offline"))
...
void envoiVersMQTT()
{
  ...
  { // Si Service MQTT activé avec période sup à 0
    previousMQTTenvoiMillis = tps;
    if (!Discovered)
    { //(uniquement au démarrage discovery = 0 et toute les 5mn si HA redemarre)
...
void callback(char *topic, byte *payload, unsigned int length)
{
  ...
  if (String(topic) == TopicP && Source == "Pmqtt")
  { // Mesure de puissance
...
void SendDataToHomeAssistant()
{
  ...
  if (TempoRTEon == 1)
  {
    sprintf(value, "%s,\"RTE_Jour\":\"%s\", \"RTE_Demain\":\"%s\"", value, RTE_Jour.c_str(), RTE_Demain.c_str());
  }
  if (Source == "Linky")
  {
  ...
  {
    if (pTriac > 0 || i > 0)
    { // On envoi pas Triac si pas présent
      ActType = "Ouverture_Relais_" + String(i);
      ActifType = "Actif_Relais_" + String(i);
      ActionDur = "Duree_Relais_" + String(i);
      ActionOnOff = "Force_OnOff_Relais_" + String(i);
